...
class TelegramCapture:
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        # `/start link-<code>` is the QR-pairing deeplink generated by the
        # Settings UI. Telegram exposes the deeplink argument via
        # context.args; arg[0] starts with "link-" when we're pairing.
        # Pairing bypasses the allowlist check on purpose — the whole point
        # is to ADD this chat to the allowlist, which is empty before pairing.
        if context.args and context.args[0].startswith("link-"):
            return await self._handle_pair(update, context.args[0])
    ...
    async def _handle_pair(self, update: Update, code: str) -> None:
        """QR-pairing handler. Consumes a one-time code and adds this chat
        to the allowlist.

        The pairing code lives in the supervisor's in-memory dict. On a
        successful consume we also persist the chat id to config.yml so
        the allowlist survives a server restart.
        """
        ...
        if chat is None:
            return
        chat_title = chat.username or chat.full_name or str(chat.id)

        entry = supervisor.get_pair(code)
        if entry is None:
            return await update.message.reply_text(
                "This pairing link has expired. Generate a new one in M3 Settings → Telegram."
            )
        if entry.chat_id is not None and entry.chat_id != chat.id:
            return await update.message.reply_text(
                "This pairing link has already been used by another chat."
            )

        # Persist the chat id to config — survives restarts.
        def _add_chat(cfg: _cfg.M3Config) -> _cfg.M3Config:
            if chat.id not in cfg.telegram.allowed_chats:
                cfg.telegram.allowed_chats = sorted({*cfg.telegram.allowed_chats, chat.id})
            return cfg
        _cfg.update(_add_chat)

        # Refresh our in-memory copy so the next message from this chat
        # passes _chat_is_allowed without waiting for a process restart.
        self.allowed_chats = frozenset({*self.allowed_chats, chat.id})

        supervisor.consume_pair_code(code, chat.id, chat_title)
        await update.message.reply_text(
            "✓ Linked to M3. Send me anything — text, photos, documents, voice notes — "
            "and I'll route it into your brain."
        )
