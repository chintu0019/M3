...
class _TitlingLLM:
    """Acts like _FinalOnlyLLM but also exposes `complete` so the auto-title
    routine can call it. Tests set the title via set_completion_response."""
...
def test_first_turn_triggers_auto_title(tmp_path):
    ...
    init_brain(brain)
    titling = _TitlingLLM()
    titling.set_completion_response("Refactor force layout")
    app = build_app(brain_root=brain, embedder=_Embedder(), llm_factory=lambda: titling)
    ...
    with c.stream(
        "POST",
        "/api/v1/chat",
        json={"message": "Help me refactor force layout", "session_id": sid},
    ) as r:
        for _ in r.iter_lines():
            pass

    # The auto-title runs as a fire-and-forget asyncio.create_task spawned
    # inside the SSE generator. TestClient's sync iter_lines exits once the
    # stream is fully drained, but the background task runs on the same
    # event loop and is awaited as part of the request lifecycle, so by the
    # time this line executes the file has been written.
